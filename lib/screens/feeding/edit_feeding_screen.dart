...
class _EditFeedingScreenState extends State<EditFeedingScreen> {
  ...
  Future<void> _loadRecord() async {
    ...
    try {
      ...
      _errorMessage = '수유 기록 정보를 불러오지 못했습니다.';
    } finally {
      if (mounted) {
        setState(() {
          _isLoading = false;
  ...
  void _applyRecord(FeedingRecord record) {
    _selectedFeedingType = record.feedingType;
    _selectedSide = record.side;
    _amountController.text = record.amount?.toString() ?? '';
    _durationController.text = record.durationMinutes?.toString() ?? '';
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      await context.read<FeedingState>().editFeedingRecord(
            widget.babyId,
            widget.recordId,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('수유 기록이 수정되었습니다.')),
      );
      context.pop();
    } catch (error) {
      if (!mounted) {
        ...
        child: Column(
          children: [
            DropdownButtonFormField<FeedingType>(
              ...
              decoration: const InputDecoration(labelText: '수유 타입'),
            ...
            if (_requiresSide(_selectedFeedingType))
              DropdownButtonFormField<String>(
                ...
                decoration: const InputDecoration(labelText: '수유 부위'),
                items: const [
            ...
            if (_requiresSide(_selectedFeedingType)) const SizedBox(height: 12),
            TextFormField(
