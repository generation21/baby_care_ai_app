...
class FeedingDetailScreen extends StatefulWidget {
  final int babyId;
  final int recordId;
  ...
  @override
  State<FeedingDetailScreen> createState() => _FeedingDetailScreenState();
}

class _FeedingDetailScreenState extends State<FeedingDetailScreen> {
  ...
  Future<void> _loadRecord() async {
    ...
    try {
      final record = await context
          .read<FeedingState>()
          .getFeedingRecord(widget.babyId, widget.recordId);
      if (!mounted) {
        return;
      }
      setState(() {
      ...
      setState(() {
        _errorMessage = '수유 기록을 불러오지 못했습니다.';
      });
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _moveToEditScreen() async {
    await context.push('/feeding/${widget.babyId}/${widget.recordId}/edit');
    if (!mounted) {
      return;
    }
    await _loadRecord();
  ...
  Future<void> _confirmAndDelete() async {
    ...
    final shouldDelete = await showDialog<bool>(
          ...
          builder: (context) => AlertDialog(
            title: const Text('수유 기록 삭제'),
            content: const Text('선택한 수유 기록을 삭제하시겠어요?\n삭제 후 되돌릴 수 없습니다.'),
            actions: [
              TextButton(
    ...
    try {
      await feedingState.deleteFeedingRecord(widget.babyId, widget.recordId);
      if (!mounted) {
        return;
      }
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('수유 기록이 삭제되었습니다.')),
      );
      context.pop();
    } catch (error) {
      if (!mounted) {
    ...
    if (_errorMessage != null || _record == null) {
      return Center(
        child: Column(
          ...
          children: [
            Text(_errorMessage ?? '수유 기록을 찾을 수 없습니다.'),
      ...
      child: Column(
        children: [
          _buildInfoCard(
            title: '기본 정보',
            rows: [
              _InfoRow(label: '타입', value: record.feedingType.displayName),
              _InfoRow(label: '기록 시각', value: recordedLabel),
              if (record.amount != null)
                _InfoRow(label: '섭취량', value: '${record.amount}${record.unit ?? ''}'),
              if (record.durationMinutes != null)
                _InfoRow(label: '수유 시간', value: '${record.durationMinutes}분'),
              if (record.side != null)
                _InfoRow(label: '부위', value: _sideLabel(record.side!)),
            ],
          ),
