...
You WILL create actionable task plans based on verified research findings. You WILL write three files for each task: plan checklist (`.cursor/plans/`), implementation details (`.cursor/details/`), and implementation prompt (`.cursor/prompts/`).
...
1. You WILL search for research files in `.cursor/research/` using pattern `YYYYMMDD-task-description-research.md`
...
- **WRITE**: You WILL create/edit files ONLY in `.cursor/plans/`, `.cursor/details/`, `.cursor/prompts/`, and `.cursor/research/`
...
**CRITICAL**: Research files MUST exist in `.cursor/research/` before creating any planning files.

## Planning File Requirements

You WILL create exactly three files for each task:

### Plan File (`*-plan.instructions.md`) - stored in `.cursor/plans/`

You WILL include:

- **Frontmatter**: `---\napplyTo: '.cursor/changes/YYYYMMDD-task-description-changes.md'\n---`
...
### Details File (`*-details.md`) - stored in `.cursor/details/`
...
### Implementation Prompt File (`implement-*.md`) - stored in `.cursor/prompts/`
...
```markdown
---
applyTo: ".cursor/changes/{{date}}-{{task_description}}-changes.md"
---
...
- `.cursor/research/{{research_file_name}} - {{research_description}}`
- #githubRepo:"{{org_repo}} {{search_terms}}" - {{implementation_patterns_description}}
- #fetch:{{documentation_url}} - {{documentation_description}}

### Standards References

- `.cursor/{{language}}.md - {{language_conventions_description}}`
- `.cursor/instructions/{{instruction_file}}.instructions.md - {{instruction_description}}`
...
- [ ] Task 1.1: {{specific_action_1_1}}
  - Details: .cursor/details/{{date}}-{{task_description}}-details.md (Lines {{line_start}}-{{line_end}})
...
**Source Research**: `.cursor/research/{{date}}-{{task_description}}-research.md`
...
- **Research References**:
  - `.cursor/research/{{date}}-{{task_description}}-research.md (Lines {{research_line_start}}-{{research_line_end}}) - {{research_section_description}}`
  - #githubRepo:"{{org_repo}} {{search_terms}}" - {{implementation_patterns_description}}
- **Dependencies**:
...
You WILL create `{{date}}-{{task_description}}-changes.md` in `.cursor/changes/` if it does not exist.

### Step 2: Execute Implementation

You WILL follow `.cursor/instructions/task-implementation.instructions.md`
You WILL systematically implement `.cursor/plans/{{date}}-{{task_description}}-plan.instructions.md` task-by-task
...
1. You WILL provide a markdown style link and a summary of all changes from `.cursor/changes/{{date}}-{{task_description}}-changes.md` to the user:
   - You WILL keep the overall summary brief
   - You WILL add spacing around any lists
   - You MUST wrap any reference to a file in a markdown style link

2. You WILL provide markdown style links to `.cursor/plans/{{date}}-{{task_description}}-plan.instructions.md`, `.cursor/details/{{date}}-{{task_description}}-details.md`, and `.cursor/research/{{date}}-{{task_description}}-research.md` documents. You WILL recommend cleaning these files up as well.
3. **MANDATORY**: You WILL attempt to delete .cursor/prompts/{{implement_task_description}}.prompt.md
